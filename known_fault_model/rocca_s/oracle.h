...
void R(uint64_t *state, uint64_t *x0, uint64_t *x1){
    ...
    /* s_new[0] = s[6]^s[1] */
    copy(&new_state[0], &state[12], 128);   xr(&new_state[0], &state[2], 128);

    /* s_new[1] = aes(s[0], x0) */
    copy(&new_state[2], &state[0], 128);    aes(&new_state[2], x0);

    /* s_new[2] = aes(s[1], s[0]) */
    copy(&new_state[4], &state[2], 128);    aes(&new_state[4], &state[0]);

    /* s_new[3] = aes(s[2], s[6]) */
    copy(&new_state[6], &state[4], 128);    aes(&new_state[6], &state[12]);

    /* s_new[4] = aes(s[3], x1) */
    copy(&new_state[8], &state[6], 128);    aes(&new_state[8], x1);
...
void initialization(uint64_t *state, uint64_t *nonce, uint64_t *k0, uint64_t *k1){
    ...
    copy(&state[8], z1, 128);
    copy(&state[10], nonce, 128);    xr(&state[10], k1, 128);
...
void enc(uint64_t *state, uint64_t *msg, uint64_t *cip){
    ...
    for (uint32_t i=0; i<m; i++){
        copy(&cip[4*i], &state[6], 128);   xr(&cip[4*i], &state[10], 128);  aes(&cip[4*i], &state[0]);
        xr(&cip[4*i], &msg[4*i], 128);

        copy(&cip[4*i + 2], &state[8], 128);    xr(&cip[4*i + 2], &state[12], 128);  aes(&cip[4*i + 2], &state[4]);
