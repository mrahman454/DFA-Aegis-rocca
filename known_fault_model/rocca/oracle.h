/* gist: This is c code for rocca. Four test vectors of the paper (link: https://tosc.iacr.org/index.php/ToSC/article/view/8904/8480) */
...
void R(uint64_t *state, uint64_t *x0, uint64_t *x1){
    ...
    copy(&new_state[0], &state[14], 128);   xr(&new_state[0], x0, 128);
    copy(&new_state[2], &state[0], 128);    aes(&new_state[2], &state[14]);
    copy(&new_state[4], &state[2], 128);    xr(&new_state[4], &state[12], 128);
    copy(&new_state[6], &state[4], 128);    aes(&new_state[6], &state[2]);

    copy(&new_state[8], &state[6], 128);    xr(&new_state[8], x1, 128);
    copy(&new_state[10], &state[8], 128);   aes(&new_state[10], &state[6]);
    copy(&new_state[12], &state[10], 128);  aes(&new_state[12], &state[8]);
    copy(&new_state[14], &state[0], 128);   xr(&new_state[14], &state[12], 128);
    
    copy(state, new_state, 8*128);
    free(new_state);
    }
...
void initialization(uint64_t *state, uint64_t *nonce, uint64_t *k0, uint64_t *k1){
    ...
    copy(&state[0], k1, 128);
    copy(&state[2], nonce, 128);
    copy(&state[4], z0, 128);
    copy(&state[6], z1, 128);

    copy(&state[8], nonce, 128);    xr(&state[8], k1, 128);
    insert(&state[10], 0x0UL, 0x0UL);
    copy(&state[12], k0, 128);
...
void enc(uint64_t *state, uint64_t *msg, uint64_t *cip){
    ...
    for (uint32_t i=0; i<m; i++){
        copy(&cip[4*i], &state[2], 128);   aes(&cip[4*i], &state[10]);
        xr(&cip[4*i], &msg[4*i], 128);

        copy(&cip[4*i + 2], &state[0], 128);    xr(&cip[4*i + 2], &state[8], 128);  aes(&cip[4*i + 2], &state[4]);
