...
/* rnd function returns a 32-bit random number using drand48() */
uint32_t rnd(void){
    float a = drand48();
    uint32_t b = (a*100000);
    b = b%0xffff;
...
void circ_shift(uint64_t *msg, uint8_t pos, uint32_t size, uint8_t str){
...
void shift(uint64_t *msg, uint8_t pos, uint32_t size, uint8_t str){
    /* 0 for the left shift */
    if (str == 0){
        uint16_t idx64 = pos/64;
        uint16_t pos64 = pos%64;
